...
class Decoder(tf.keras.Model):
    def __init__(self, embedding_matrix, dec_units, batch_sz):
        ...
        # self.rnn = tf.keras.layers.RNN(self.dec_units, return_sequences=True, return_state=True)
        # self.lstm = tf.keras.layers.LSTM(self.dec_units, return_sequences=True, return_state=True)
        """
        定义最后的fc层，用于预测词的概率
        请写出你的代码
        """
        self.fc = tf.keras.layers.Dense(self.dec_units, activation='tanh')
        """
        注意力机制
        请写出你的代码
        """
